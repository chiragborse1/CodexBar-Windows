...
#if os(Windows)
private typealias ProcessGroupID = Int32
#else
private typealias ProcessGroupID = pid_t
#endif
...
public enum SubprocessRunner {
    ...
    /// Terminates a process and, on POSIX systems, its process group.
    /// Returns `true` if the process was actually killed, `false` if it had already exited.
    @discardableResult
    private static func terminateProcess(_ process: Process, processGroup: ProcessGroupID?) -> Bool {
        ...
        label: String) async throws -> SubprocessResult
    {
        ...
        #if os(Windows)
        let processGroup: ProcessGroupID? = nil
        #else
        let pid = process.processIdentifier
        let processGroup: ProcessGroupID? = setpgid(pid, pid) == 0 ? pid : nil
        #endif
