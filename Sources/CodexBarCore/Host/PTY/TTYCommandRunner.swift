...
typealias WindowsProcessID = Int32

private enum TTYCommandRunnerActiveProcessRegistry {
    private static let lock = NSLock()
    private nonisolated(unsafe) static var processes: [WindowsProcessID: ProcessInfo] = [:]
    private nonisolated(unsafe) static var isShuttingDown = false

    private struct ProcessInfo {
        let binary: String
        var processGroup: WindowsProcessID?
    }

    @discardableResult
    static func register(pid: WindowsProcessID, binary: String) -> Bool {
    ...
    static func updateProcessGroup(pid: WindowsProcessID, processGroup: WindowsProcessID?) {
    ...
    static func unregister(pid: WindowsProcessID) {
        guard pid > 0 else { return }
        self.lock.lock()
    ...
    static func drainForShutdown() -> [(pid: WindowsProcessID, binary: String, processGroup: WindowsProcessID?)] {
        self.lock.lock()
    ...
    static func testTrackProcess(pid: WindowsProcessID, binary: String, processGroup: WindowsProcessID?) {
        guard pid > 0 else { return }
        self.lock.lock()
...
public struct TTYCommandRunner {
    ...
    public static func terminateActiveProcessesForAppShutdown() {
        _ = TTYCommandRunnerActiveProcessRegistry.drainForShutdown()
    }

    @discardableResult
    static func registerActiveProcessForAppShutdown(pid: WindowsProcessID, binary: String) -> Bool {
        TTYCommandRunnerActiveProcessRegistry.register(pid: pid, binary: binary)
    }

    static func updateActiveProcessGroupForAppShutdown(pid: WindowsProcessID, processGroup: WindowsProcessID?) {
        TTYCommandRunnerActiveProcessRegistry.updateProcessGroup(pid: pid, processGroup: processGroup)
    }

    static func unregisterActiveProcessForAppShutdown(pid: WindowsProcessID) {
        TTYCommandRunnerActiveProcessRegistry.unregister(pid: pid)
    }

    private static func resolveShutdownTargets(
        _ targets: [(pid: WindowsProcessID, binary: String, processGroup: WindowsProcessID?)],
        hostProcessGroup: WindowsProcessID,
        groupResolver: (WindowsProcessID) -> WindowsProcessID)
        -> [(pid: WindowsProcessID, binary: String, processGroup: WindowsProcessID?)]
    {
        var resolvedTargets: [(pid: WindowsProcessID, binary: String, processGroup: WindowsProcessID?)] = []
    ...
    static func drainRemainingOutput(
        ...
        sleep: (UInt32) -> Void = { Thread.sleep(forTimeInterval: TimeInterval($0) / 1_000_000) })
    {
    ...
    static func drainReadResult(for data: Data, terminalRead: Int, errno _: Int32) -> DrainReadResult {
        if !data.isEmpty { return .data(data) }
        if terminalRead == 0 { return .closed }
        if terminalRead < 0 { return .wouldBlock }
        return .closed
    }

    static func locateBundledHelper(_ name: String) -> String? {
        let key = "CODEXBAR_HELPER_\(name.uppercased())"
    ...
    public static func which(_ tool: String) -> String? {
        ...
        return self.findExecutable(tool, env: ProcessInfo.processInfo.environment)
    }

    /// Uses the Windows PATH so probes match the user's command-line environment.
    ...
    private static func findExecutable(_ tool: String, env: [String: String]) -> String? {
        let fileManager = FileManager.default

        if tool.contains("\\") || tool.contains("/") {
            for candidate in self.executableCandidateNames(for: tool, env: env)
        ...
        guard let path = env["PATH"], !path.isEmpty else { return nil }
        let paths = path.split(separator: ";").map(String.init)
        for directory in paths where !directory.isEmpty {
            for candidateName in self.executableCandidateNames(for: tool, env: env) {
                let candidate = self.join(path: directory, component: candidateName)
    ...
    private static func executableCandidateNames(for tool: String, env: [String: String]) -> [String] {
        let pathext = env["PATHEXT"]?
            .split(separator: ";")
            .map { String($0).trimmingCharacters(in: .whitespacesAndNewlines) }
            .filter { !$0.isEmpty } ?? [".exe", ".cmd", ".bat", ".ps1"]
        let extensions = pathext.map { $0.hasPrefix(".") ? $0 : ".\($0)" }
        let lowercased = tool.lowercased()
        if extensions.contains(where: { lowercased.hasSuffix($0.lowercased()) }) {
            return [tool]
        }
        return [tool] + extensions.map { tool + $0 }
    }

    private static func join(path: String, component: String) -> String {
        var trimmed = path
    ...
    static func _test_trackProcess(pid: WindowsProcessID, binary: String, processGroup: WindowsProcessID?) {
    ...
    @discardableResult
    static func _test_registerTrackedProcess(pid: WindowsProcessID, binary: String) -> Bool {
        TTYCommandRunnerActiveProcessRegistry.register(pid: pid, binary: binary)
    }
