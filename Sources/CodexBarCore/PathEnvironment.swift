...
public enum BinaryLocator {
        ...
        home: String) -> String?
    {
        ...
        if let existingPATH = env["PATH"],
           let pathHit = self.find(
               name,
               in: self.pathList(from: existingPATH),
               fileManager: fileManager)
        {
            return pathHit
        }
        ...
        // 6) Minimal fallback
        #if os(Windows)
        let fallback = ["C:\\Windows\\System32", "C:\\Windows"]
        #else
    ...
    private static func pathList(from value: String) -> [String] {
        #if os(Windows)
    ...
    private static func find(_ binary: String, in paths: [String], fileManager: FileManager) -> String? {
        for path in paths where !path.isEmpty {
            for candidateName in self.executableCandidateNames(for: binary) {
                let candidate = self.join(path: path, component: candidateName)
    ...
    private static func executableCandidateNames(for binary: String) -> [String] {
        #if os(Windows)
        let lowercased = binary.lowercased()
        let knownExtensions = [".exe", ".cmd", ".bat", ".ps1"]
        if knownExtensions.contains(where: { lowercased.hasSuffix($0) }) {
            return [binary]
        }
        return [binary] + knownExtensions.map { binary + $0 }
        #else
        return [binary]
        #endif
    }
    ...
    private static func join(path: String, component: String) -> String {
        ...
        #else
        return "\(path.hasSuffix("/") ? String(path.dropLast()) : path)/\(component)"
        #endif
    }
}
...
public enum PathBuilder {
        ...
        home _: String = NSHomeDirectory()) -> String
    {
        ...
        if let existing = env["PATH"], !existing.isEmpty {
            parts.append(contentsOf: self.pathList(from: existing))
        }

        if parts.isEmpty {
            #if os(Windows)
            parts.append(contentsOf: ["C:\\Windows\\System32", "C:\\Windows"])
            #else
        ...
        return deduped.joined(separator: self.separator)
    }
        ...
        home: String = NSHomeDirectory()) -> PathDebugSnapshot
    {
        ...
        let loginString = login?.joined(separator: self.separator)
